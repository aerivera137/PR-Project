...
function hurricane_sim!(model::Model, inputs::Dict)
    ...
    Z = 1:inputs["Z"]
    L = 1:inputs["L"]
    BL = process_BL_inputs(inputs,dfGen)
    ...
    function check_substations(z, MG_Zone, generators, original_load, P_HURRICANE, category)
        ...
        # Create array of all generators where a value of '1' means the microgrid is used, '0' means unused
        array_MG_Used = zeros(length(G))
        ...
        if (num_MG != 0)
            ...
            for mg in MG_Zone
                ...
                if (rand(Bernoulli(get_failure_rate(generators,mg,category)))==0) && (dfGen[mg,"Existing_Cap_MW"]>0)
        ...
        # Check substations for failure
        # Iterate through remaining substations
        n = 1 # index for set of indices for available microgrids
        for i in 1:num_substations
            # Check if substation has failed
            if rand(Bernoulli(Substation_FailRates.Failure_Rate[category]))
                # Check if a microgrid can protect that substation
                if num_MG_Available > 0
                    println("There is a microgrid protecting a substation!")
                    used_MG_Index = indices_MG_Available[n]
                    array_MG_Used[used_MG_Index] = 1
                    num_MG_Available -= 1
                    n += 1
                else
                    # If no microgrid, remove portion of demand for substation
    ...
    function damage_sim(category,P_HURRICANE)
        ...
        for l in BL
            ...
            if rand(Bernoulli(BurLine_FailRates.Failure_Rate[category]))
                # If failed, cut off power flow in this line during the hurricane
                println("A buried transmission line has failed - modifying power flow")
                START = first(P_HURRICANE)
                END = last(P_HURRICANE)
                restoration_time = 336 # two weeks = 336 hours
                outage_time = START:(END+restoration_time)
    ...
    return category, P_HURRICANE, outages, line_availability, dfNSE, dfNSEHurricane
end